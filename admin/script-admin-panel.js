...
(function() {
    ...
    // ============================================================
    // 0505 - عملیات گیت‌هاب (نسخه اصلی - بدون پارامتر اضافی)
    // ============================================================
    ...
    // ============================================================
    // 0508 - مقالات (با مدیریت خطای JSON)
    // ============================================================
    ...
    // ============================================================
    // 0509 - محصولات (با مدیریت خطای JSON)
    // ============================================================
    ...
    // ============================================================
    // 0510 - آرشیو (با مدیریت خطای JSON)
    // ============================================================
    var archiveFiltered = [];
    ...
    // ============================================================
    // 0515 - ابزارهای ویرایشگر متن + توابع تولید محتوا
    // ============================================================
    ...
    // ---- تابع ذخیره مقاله (برای blank-article) ----
    async function saveArticle() {
        ...
        var difficulty = document.getElementById('articleDifficulty')?.value || 'متوسط';
        var language = document.getElementById('articleLanguage')?.value || 'فارسی';
        var englishAbstract = document.getElementById('articleEnglishAbstract')?.value?.trim() || '';
        var status = document.getElementById('articleStatus')?.value || 'فعال';
        ...
        var galleryData = Array.from(galleryItems).map(function(img) { return img.src; });
        ...
        var filesData = Array.from(fileTags).map(function(tag) {
            var name = tag.querySelector('span')?.textContent || '';
            return name;
        });

        var articleData = {
            title: title, abstract: abstract, body: body, type: type, date: date, year: year, readTime: readTime,
            tags: tags, keywords: keywords, cover: coverData || '', images: galleryData,
            file: mainFileName || '', files: filesData,
            references: references, doi: doi, difficulty: difficulty, language: language,
            english_abstract: englishAbstract, status: status,
        ...
        try {
            ...
            var repoOwner = 'mahanneman';
            var repoName = 'MA.AD.GH.SITE';
    ...
    // ---- تابع بازنشانی فرم مقاله ----
    function resetArticleForm() {
        var fields = ['articleTitle', 'articleAbstract', 'articleBody', 'articleDate', 'articleYear', 'articleReadTime',
            'articleKeywords', 'articleReferences', 'articleDoi', 'articleEnglishAbstract', 'articleNotes', 'articleRelatedLinks'
        ];
        ...
        var diffEl = document.getElementById('articleDifficulty');
        if (diffEl) diffEl.value = 'متوسط';
        var langEl = document.getElementById('articleLanguage');
        if (langEl) langEl.value = 'فارسی';
        var statusEl = document.getElementById('articleStatus');
        if (statusEl) statusEl.value = 'فعال';
        var tagsEl = document.getElementById('articleTags');
        if (tagsEl) tagsEl.value = '';
        var container = document.getElementById('articleTagsContainer');
        if (container) container.querySelectorAll('.tag-item').forEach(function(el) { el.remove(); });
        var previews = ['coverPreview', 'galleryPreview', 'mainFilePreview', 'filesPreview'];
        previews.forEach(function(id) {
            var el = document.getElementById(id);
            if (el) el.innerHTML = '';
        });
        var inputs = ['coverFileInput', 'galleryFileInput', 'mainFileInput', 'filesFileInput'];
    ...
    function updateTagsHidden(containerId) {
        var container = document.getElementById(containerId);
        if (!container) return;
        var tags = container.querySelectorAll('.tag-item');
        var values = Array.from(tags).map(function(t) { return t.textContent.replace('×', '').trim(); });
        var hiddenId = containerId.replace('Container', '');
        var hiddenEl = document.getElementById(hiddenId);
        if (hiddenEl) hiddenEl.value = values.join(',');
    }

    // ---- توابع آپلود برای blank-article ----
    ...
    function initArticleUploads() {
        // آپلودهای فرم اصلی (در صورت وجود)
        if (document.getElementById('articleFilesZone')) {
            setupFileUpload('articleFilesZone', 'articleFilesInput', 'articleFilesList', 'article', 10);
        }
        if (document.getElementById('articleGalleryZone')) {
            setupFileUpload('articleGalleryZone', 'articleGalleryInput', 'articleGalleryList', 'article_gallery', 4);
        }
        if (document.getElementById('articleCoverZone')) {
            setupCoverUpload('articleCoverZone', 'articleCoverInput', 'articleCoverPreview', 'articleCoverPreviewImg', 'removeArticleCover');
        }
        // برای blank-article نیز راه‌اندازی می‌کنیم
    ...
    function updateDashboard() {
        ...
        // به‌روزرسانی تعداد کاربران در داشبورد
    ...
    async function refreshMembers() { await loadMembers();
