...
// ============================================================
document.getElementById('addArchiveForm').addEventListener('submit', async function(e) {
    e.preventDefault();
...
function loadMenuData() {
    ...
    fetchFromGitHub('_data/menu.json').then(data => {
        if (data) {
            try { menuData = JSON.parse(data.content); } catch (e) { menuData = { header: [], slide: [] }; }
        } else {
            menuData = { header: [], slide: [] };
            saveToGitHub('_data/menu.json', menuData, null).catch(() => {});
        }
        renderMenuLists();
    }).catch((err) => {
...
function addMenuItem(type) {
    ...
    if (!title) { showMsg('لطفاً عنوان آیتم را وارد کنید.', 'error'); return; }
...
function openEditMenuModal(type, index) {
    ...
    document.getElementById('editMenuForm').addEventListener('submit', function(e) {
        ...
        if (!newTitle) { showMsg('❌ لطفاً عنوان را وارد کنید.', 'error'); return; }
    ...
    modal.innerHTML = `
        <div class="pro-modal" style="max-width:700px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:12px;background:var(--pro-bg);border-radius:var(--pro-radius);border:1px solid var(--pro-border);">
                ...
                <div><span style="color:var(--pro-text-secondary);">وضعیت:</span>
                    <select id="orderStatusSelect" style="padding:4px 8px;border-radius:6px;background:var(--pro-bg);color:var(--pro-text);border:1px solid var(--pro-border);">
                    ...
                    <button class="pro-btn pro-btn-sm pro-btn-primary" onclick="updateOrderStatus('${memberId}', ${orderIndex})" style="margin-right:8px;"><i class="fas fa-save"></i> تغییر</button>
                </div>
                <div><span style="color:var(--pro-text-secondary);">آدرس:</span> <strong>${order.address || '---'}</strong></div>
                <div><span style="color:var(--pro-text-secondary);">روش ارسال:</span> <strong>${order.shipping || '---'}</strong></div>
                <div><span style="color:var(--pro-text-secondary);">کد پیگیری:</span> <strong>${order.tracking || '---'}</strong></div>
                <div><span style="color:var(--pro-text-secondary);">مبلغ کل:</span> <strong style="color:var(--pro-primary);">${(order.total || 0).toLocaleString()} تومان</strong></div>
            </div>
            <h4 style="margin-top:16px;margin-bottom:8px;">محصولات</h4>
            <div style="background:var(--pro-bg);border-radius:var(--pro-radius);border:1px solid var(--pro-border);padding:12px;">
                ${(order.items || []).map(item => `
                    <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--pro-border);font-size:0.9rem;">
...
async function updateOrderStatus(memberId, orderIndex) {
    ...
    try {
        ...
        let sha = null;
        if (existing) sha = existing.sha;
        await saveToGitHub(path, currentMemberOrders, sha);
